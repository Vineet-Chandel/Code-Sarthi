...
// We need to extract the comment rendering into a CommentItem component.
// I will just rewrite the whole file since it's cleaner.
...
const IssueComments = ({ issueId, teamId, currentUserId }) => {
    ...
    return (
        <div className="flex flex-col mt-8 border-t border-white/[0.04] pt-6 max-w-full">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar max-h-[300px] space-y-4 mb-4 pr-2 max-w-full">
                {loading ? (
                    ...
                    <AnimatePresence initial={false}>
                        {comments.map(comment => (
                            <CommentItem 
                                key={comment._id}
                                comment={comment}
                                isAuthor={currentUserId && comment.authorId?._id === currentUserId}
            ...
            <form onSubmit={handlePost} className="relative mt-auto pt-2 w-full max-w-full">
                <div className="bg-[#111111] border border-white/[0.06] focus-within:border-white/[0.15] focus-within:shadow-[0_0_20px_rgba(255,255,255,0.02)] rounded-2xl overflow-hidden transition-all duration-300 relative group max-w-full">
                    <textarea
                        ...
                        className="w-full bg-transparent text-[13px] text-zinc-200 p-4 pb-12 focus:outline-none resize-none custom-scrollbar min-h-[90px] leading-relaxed placeholder:text-zinc-600 block"
                        onKeyDown={(e) => {
