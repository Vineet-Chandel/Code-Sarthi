...
const categories = [
    ...
    },
    {
        id: 'website',
        name: 'Website Link',
        description: 'Design file, staging environment, or reproduction link.',
        icon: (
    ...
    },
    {
        ...
        description: 'Slack, Discord, Twitter/X, or community discussion.',
        icon: (
...
const IssueLinksModal = ({ isOpen, onClose, issue, myRole, onAddLink, onRemoveLink, loading }) => {
    ...
    const canManage = myRole === 'leader' || myRole === 'admin' || (issue && user && (String(issue.createdBy) === String(user._id) || (issue.assignedTo && String(issue.assignedTo._id || issue.assignedTo) === String(user._id))));
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-zinc-400 truncate max-w-[250px] sm:max-w-md">
                                        {issue?.title ? `Attached resources for ${issue.title}` : 'Manage attached issue URLs'}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                    ...
                                    {links.length === 0 ? (
                                        <div className="bg-[#000000] border border-dashed border-white/[0.08] rounded-2xl p-10 text-center flex flex-col items-center justify-center my-2">
                                            ...
                                            <p className="text-xs text-zinc-400 max-w-sm mb-5">
                                                Attach relevant Pull Requests, design files, screenshot links, or documentation directly to this issue.
                                            </p>
                                            <button
                                                onClick={() => setIsAdding(true)}
...
export default IssueLinksModal;
