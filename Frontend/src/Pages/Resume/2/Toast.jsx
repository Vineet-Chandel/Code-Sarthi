...
const themes = {
    error: {
        ...
        BlobSvg: ({ dark }) => (
            <svg width="150" height="105" className="rounded-bl-[35px]" viewBox="0 0 93 87" fill="none">
                <circle cx="33.9429" cy="36.8292" r="6.82918" fill={dark} />
                <circle cx="79" cy="27" r="13" fill={dark} />
                <path fillRule="evenodd" clipRule="evenodd" d="M32.5511 49.4389C32.4191 48.7782 31.9357 48.2428 31.2918 48.0442C30.5686 47.8212 29.7827 48.0647 29.3124 48.6576L26.6097 52.0648C24.7585 54.3984 21.7904 55.5017 18.8235 55.2372C11.5519 54.5888 4.04597 55.1701 -3.42021 57.1407C-37.9518 66.255 -58.5567 101.637 -49.4425 136.169C-40.3282 170.7 -4.9463 191.305 29.5853 182.191C64.1169 173.077 84.7218 137.695 75.6076 103.163C70.3813 83.3617 56.5177 68.1397 39.13 60.4748C36.2342 59.1983 34.0008 56.6968 33.3809 53.5935L32.5511 49.4389Z" fill={dark} />
                <circle cx="21.5475" cy="4.5476" r="3.49096" transform="rotate(-22.0902 21.5475 4.5476)" fill={dark} />
            </svg>
        ),
    },
    success: {
        ...
        BlobSvg: ({ dark }) => (
            <svg width="150" height="105" className="rounded-bl-[35px]" viewBox="0 0 93 87" fill="none">
                <circle cx="20.6075" cy="9.29547" r="9.29547" fill={dark} />
                <circle cx="80.0986" cy="47.7167" r="3.71819" fill={dark} />
                <path d="M79.4438 11.0253C82.4971 18.5483 78.8737 27.1221 71.3507 30.1754C70.5208 30.5122 69.6781 30.7678 68.8315 30.9458C64.1204 31.9366 58.8591 33.2841 56.3382 37.3855C53.3951 42.1741 55.0036 48.3927 59.3496 51.9571C68.015 59.0642 75.0268 68.4315 79.3829 79.6187C92.9059 114.348 75.7149 153.464 40.9856 166.987C6.25636 180.51 -32.8599 163.319 -46.3829 128.59C-59.9059 93.8607 -42.7149 54.7445 -7.98562 41.2214C7.18342 35.3148 23.1894 35.2678 37.5341 39.9824C42.7299 41.69 48.6536 40.072 51.5174 35.4125L52.5823 33.68C54.694 30.2441 53.7172 25.8191 52.2006 22.0823C49.1473 14.5592 52.7707 5.98544 60.2937 2.93215C67.8167 -0.121136 76.3906 3.5023 79.4438 11.0253Z" fill={dark} />
            </svg>
        ),
    },
    warning: {
...
const Toast = ({ type = "error", title = "Oh snap!", message = "Change a few things up and try submitting again.", onClose }) => {
    const [paused, setPaused] = useState(false);
    const theme = themes[type] ?? themes.error;
    const { BubbleIcon, BlobSvg, bg, dark } = theme;
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0, y: 20, scale: 0.93, transition: { duration: 0.2 } }}
            onMouseEnter={() => setPaused(true)}
            onMouseLeave={() => setPaused(false)}
            style={{ backgroundColor: bg, fontFamily: "'Poppins', sans-serif" }}
            className="pointer-events-auto relative h-[160px]  rounded-[35px] flex items-center justify-between text-white shadow-2xl"
        >
            {/* ── Left: floating bubble icon + bottom blob ── */}
            <div className="w-[150px]  self-stretch flex-shrink-0 relative">
                {/* Bouncing bubble — floats above card */}
                <div className="relative top-[-30px] left-[25px]  z-30">
                    <BubbleIcon />
                </div>
                {/* Decorative blob locked to bottom-left */}
                <div className="absolute bottom-0 left-0 rounded-bl-[35px] overflow-hidden">
                    <BlobSvg dark={dark} />
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-1 pr-2">
                <div className="font-medium text-2xl sm:text-3xl md:text-[38px] leading-none tracking-[-0.035em]  truncate">
                    {title}
                </div>
                <div className="font-extralight text-[14px] tracking-[0.02em] leading-snug opacity-90">
                    {message}
                </div>
            </div>
            ...
            <button
                onClick={onClose}
                className="self-start mt-6 mr-6 flex-shrink-0 text-white "
            >
                <svg
                ...
                >
                    ...
                    <path fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="4" d="M20 20L4 4m16 0L4 20" />
                </svg>
            </button>

            {/* ── Progress bar ── */}
            <div className="absolute bottom-0 left-5 h-[4px] w-full bg-black/10 ">
                <motion.div
                    initial={{ width: "90%" }}
